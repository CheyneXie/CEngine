...
// IBL_Prefilter.frag — GGX 重要性采样预过滤环境立方图
//
...
void main()
{
    vec3 N = normalize(FSIn.WorldDir);
    ...
    for (uint i = 0u; i < SAMPLE_COUNT; ++i)
    {
        ...
        if (NdotL > 0.0)
        {
            ...
            float resolution = 128.0; // 与 prefilterCube 每面尺寸对应
            float saTexel = 4.0 * PI / (6.0 * resolution * resolution);
            float saSample = 1.0 / (float(SAMPLE_COUNT) * D * D + 1.0);
